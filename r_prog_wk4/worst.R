worst <- function(state, outcome){
  ...
  if(outcome == 'heart attack'){
    ...
    state_res <- state_frame$Hospital.Name[state_frame[,11] == max(rem_Na(state_frame[,11]))]}
  if(outcome == 'heart failure'){
    state_frame[,17] <- as.numeric(state_frame[,17])
    state_res <- state_frame$Hospital.Name[state_frame[,17] == max(rem_Na(state_frame[,17]))]}
  if(outcome == 'pneumonia'){
    state_frame[,23] <- as.numeric(state_frame[,23])
    state_res <- state_frame$Hospital.Name[state_frame[,23] == max(rem_Na(state_frame[,23]))]}
